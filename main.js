...
var authFolder = storeSystem.fixFileName(`${Helper.opts._[0] || ''}AmeliaSession`)
...
function clearSessions(folder = "AmeliaSession") {
let filename = [];
...
let prekey = [];
const directorio = readdirSync('./AmeliaSession');
const filesFolderPreKeys = directorio.filter((file) => {
return file.startsWith('pre-key-');
});
prekey = [...prekey, ...filesFolderPreKeys];
filesFolderPreKeys.forEach((files) => {
unlinkSync(`./AmeliaSession/${files}`);
});
}
...
const directories = ['./AmeliaSession/', './jadibot/'];
